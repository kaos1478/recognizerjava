...
public class ControlPerson {
    ...
    public void preencherTabela(String SQL, JTable tabela) {
        ...
        String[] Colunas = new String[]{"Users", "ID", "Nome", "Cargo"};
    ...
    public void update(ModelPerson mod, int id) {
        conecta.conexao();
        try {
            PreparedStatement pst = conecta.conn.prepareStatement("UPDATE person SET first_name= ?, last_name= ?, office= ?, dob= ? WHERE id=?");
    ...
    class ImageRenderer implements TableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
            try {
                ...
                ImageIcon i = new ImageIcon(new ImageIcon(String.valueOf(path)).getImage().getScaledInstance(lbl.getWidth() + 110, lbl.getHeight() + 100, Image.SCALE_SMOOTH));
                lbl.setIcon(i);
            } catch (Exception e) {
                e.printStackTrace();
            }
